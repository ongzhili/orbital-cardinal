...
export default class Deck extends Model {
  ...
  static async update(deck) {
    let dec = null
    try {
      dec = await database.get('deck').find(deck.id)
    } catch (e) {
      let collection = database.get('deck')
      return await collection.create(record => {
        record._raw = sanitizedRaw({ id: deck.id, name: deck.name, description: deck.description }, collection.schema)
      })
    }
